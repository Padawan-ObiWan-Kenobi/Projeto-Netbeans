...
public class DaoCadUsuario {
    ...
    public void Salvar (BeansCadUsuario mod) {
        conecta.conexao();
        try {
            PreparedStatement pst = conecta.Con.prepareStatement("insert into usuarios(datacadastro_usuario,nome_usuario,tipo_usuario,senha_usuario,pergunta_secreta,resposta) values(?,?,?,?,?;?)");
    ...
    public void Alterar (BeansCadUsuario mod) {
        conecta.conexao();
        try {
            PreparedStatement pst = conecta.Con.prepareStatement("update usuarios set datacadastro_usuario=?,nome_usuario=?,tipo_usuario=?,senha_usuario=?,pergunta_secreta=?,resposta=? where id_usuario=?");
            pst.setString(1, mod.getDataCadastro());           
            pst.setString(2, mod.getUsuario());
            pst.setString(3, mod.getTipoUsuario());
            pst.setString(4, mod.getSenha());
    ...
    public BeansCadUsuario BuscaUsuario(BeansCadUsuario mod) {
        ...
        try {
            ...
            mod.setPergunta(conecta.rs.getString("pergunta_secreta"));
            mod.setResposta(conecta.rs.getString("resposta"));
